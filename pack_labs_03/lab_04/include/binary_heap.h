...
typedef struct { \
    ...
    HEAP_TYPE (*copy_func)(const HEAP_TYPE); /* конструктор копирования */ \
...
Heap_##HEAP_TYPE create_heap_##HEAP_TYPE(size_t initial_capacity, void (*delete_func)(HEAP_TYPE), HEAP_TYPE (*copy_func)(const HEAP_TYPE),\
...
int is_empty_heap_##HEAP_TYPE(const Heap_##HEAP_TYPE *h) {\
    return h->size == 0; \
}\
/* Возврат текущего количества элементов */ \
size_t size_heap_##HEAP_TYPE(const Heap_##HEAP_TYPE *h) { \
    return h->size;\
} \
/* Получение элемента с наивысшим приоритетом (минимального) без удаления */ \
HEAP_TYPE peek_heap_##HEAP_TYPE(const Heap_##HEAP_TYPE *h) { \
    if (h->size == 0) { \
...
/* Просеивание вниз */ \
void sift_down_##HEAP_TYPE(Heap_##HEAP_TYPE *h, const size_t index) { \
...
/* Просеивание вверх */ \
void sift_up_##HEAP_TYPE(Heap_##HEAP_TYPE *h, const size_t index) { \
...
void push_heap_##HEAP_TYPE(Heap_##HEAP_TYPE *h, HEAP_TYPE value) { \
...
int is_equal_heap_##HEAP_TYPE(const Heap_##HEAP_TYPE *h1, const Heap_##HEAP_TYPE *h2) { \
    if (h1->size != h2->size) { \
        return 0; \
    } \
...
void delete_heap_##HEAP_TYPE(Heap_##HEAP_TYPE *h) { \
    for (size_t i = 0; i < h->size; ++i) { \
        pop_heap_##HEAP_TYPE(h); /* авоматически освобождает память */ \
    }\
    free(h->data); \
    h->capacity = 0; \
} \
